...
/**
 * Calculates a lead's score (0-100) based on statuses and data completeness.
 * Matches the frontend scoring logic exactly.
 */
const calculateLeadScore = (lead) => {
    ...
    if (hasValue(lead.email))
        score += 3;
    if (hasValue(lead.email2))
        score += 1;
    if (hasValue(lead.phone))
        score += 3;
    if (hasValue(lead.phone2))
        score += 1;
    // 6. Registration & Business Info (Max 10 points)
    if (hasValue(lead.registrationNo))
        score += 5;
    if (hasValue(lead.type) && String(lead.type).toLowerCase() !== 'manual')
        score += 3;
    if (hasValue(lead.contactPerson))
        score += 2;
    // 7. Location Info (Max 6 points)
    if (hasValue(lead.address))
        score += 2;
    if (hasValue(lead.city))
        score += 2;
    ...
    if (hasValue(lead.website))
        score += 4;
    if (hasValue(lead.logoUrl))
        score += 2;
    if (hasValue(lead.linkedin) || hasValue(lead.facebook) || hasValue(lead.twitter))
        score += 2;
    // 9. Enrichment Content (Max 14 points)
    if (lead.isEnriched)
        score += 3;
    if (hasValue(lead.servicesSummary))
        score += 3;
    if (hasValue(lead.productsOffered))
        score += 2;
    if (hasValue(lead.enrichmentNotes))
        score += 2;
    if (lead.sellsAlgoTrading === 'Yes')
        score += 2;
    if (hasValue(lead.brokerPartner))
        score += 2;
