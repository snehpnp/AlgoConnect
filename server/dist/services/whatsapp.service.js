...
class WhatsAppService {
    ...
    constructor() {
        ...
        this.client = new whatsapp_web_js_1.Client({
            authStrategy: new whatsapp_web_js_1.LocalAuth({ dataPath: './whatsapp-auth' }),
            puppeteer: {
                headless: true,
    ...
    async sendMessage(phoneNumber, text, mediaPath) {
        if (!this.isConnected)
        ...
        try {
            if (mediaPath) {
                contentToSend = whatsapp_web_js_1.MessageMedia.fromFilePath(mediaPath);
                if (text) {
            ...
            if (errMsg.includes('Lid is missing') ||
    ...
    async downloadMediaWithRetry(message, retries = 3, delayMs = 1500) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                if (attempt === retries)
                    return null; // don't throw, just give up quietly
            }
    ...
    async handleIncomingMessage(message) {
        try {
            ...
            const text = message.body || (message.hasMedia ? '[Media Message]' : '');
            if (!text && !mediaUrl)
                return;
            ...
            const existing = await prismaClient_1.default.messageSend.findUnique({
            ...
            const allLeads = await prismaClient_1.default.lead.findMany({
            ...
            let lastMessage = await prismaClient_1.default.messageSend.findFirst({
                where: {
            ...
            if (!lastMessage) {
                lastMessage = await prismaClient_1.default.messageSend.create({
                    data: {
                        leadId: matchedLead.id,
                        channel: 'WHATSAPP',
            ...
            socket_service_1.SocketService.broadcast('new_reply', {
                leadId: matchedLead.id,
                channel: 'WHATSAPP',
                text: text,
            ...
            const usersToNotify = await prismaClient_1.default.user.findMany({ where: { role: { name: 'SUPERADMIN' } } });
            for (const u of usersToNotify) {
                ...
                socket_service_1.SocketService.sendToUser(u.id, 'new_notification', notif);
            }
