...
exports.scrapeWebsiteContactInfo = exports.findWebsiteFromSearch = exports.extractPhones = exports.extractEmails = exports.fetchPageContent = void 0;
const SKIP_DOMAINS = [
...
const isValidEmail = (emailStr) => {
    ...
    if (clean.length < 6 || !clean.includes('@') || !clean.includes('.'))
        return false;
    if (/\.(png|jpg|jpeg|gif|svg|css|js|webp|woff2?)$/i.test(clean))
        return false;
    if (clean.includes('example.com') ||
...
const fetchPageContent = async (urlStr, timeoutMs = 10000) => {
...
const extractEmails = (html, preferredDomain) => {
...
const findWebsiteFromSearch = async (name, registrationNo) => {
...
exports.findWebsiteFromSearch = findWebsiteFromSearch;
const scrapeWebsiteContactInfo = async (website) => {
    ...
    const htmlChunks = await Promise.all(uniquePages.map((url) => (0, exports.fetchPageContent)(url, 8000)));
    const html = htmlChunks.filter(Boolean).join('\n');
    const emails = (0, exports.extractEmails)(html, origin);
    const phones = (0, exports.extractPhones)(html);
    return {
...
exports.scrapeWebsiteContactInfo = scrapeWebsiteContactInfo;
