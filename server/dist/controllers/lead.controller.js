...
exports.sendDirectMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    let targetRecipient = '';
    if (channel === 'EMAIL') {
        targetRecipient = recipientEmail || lead.email || lead.scrapedEmail;
        if (!targetRecipient) {
        ...
        targetRecipient = lead.phone || lead.phone2 || lead.scrapedPhone;
        if (!targetRecipient) {
...
exports.getLeadMessageHistory = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
exports.rescrapeBouncedLeads = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const bouncedLeads = await prismaClient_1.default.lead.findMany({
        where: whereClause,
    ...
    const isValidEmailFormat = (emailStr) => {
        ...
        if (clean.length < 6 || !clean.includes('@') || !clean.includes('.'))
            return false;
        if (clean.includes('mailer-daemon') || clean.includes('postmaster') || clean.includes('wixpress') || clean.includes('sentry') || clean.includes('example.com'))
            return false;
        if (/\.(png|jpg|jpeg|gif|svg|css|js|webp)$/i.test(clean))
            return false;
        return true;
    };
    const fetchPageContent = async (urlStr, timeoutMs = 3000) => {
        try {
            const httpModule = urlStr.startsWith('https') ? require('https') : require('http');
            return await new Promise((resolve) => {
                const timer = setTimeout(() => resolve(''), timeoutMs);
                const req = httpModule.get(urlStr, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' } }, (res) => {
                    let data = '';
                    res.on('data', (chunk) => { data += chunk; if (data.length > 200000)
                        res.destroy(); });
    ...
    for (const lead of bouncedLeads) {
        try {
            ...
            if (newEmailFound) {
                ...
                await prismaClient_1.default.messageSend.updateMany({
                    where: {
                        leadId: lead.id,
