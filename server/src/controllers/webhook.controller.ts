...
export const handleEmailWebhook = async (req: Request, res: Response) => {
  try {
    ...
    for (const event of events) {
      ...
      if (rawType.includes('DELIVER')) eventType = 'DELIVERED';
      else if (rawType.includes('OPEN')) eventType = 'OPENED';
      else if (rawType.includes('CLICK')) eventType = 'CLICKED';
      else if (rawType.includes('BOUNCE')) eventType = 'BOUNCED';
      else if (rawType.includes('SPAM')) eventType = 'SPAM';
      else if (rawType.includes('UNSUBSCRIBE')) eventType = 'UNSUBSCRIBED';
      else if (rawType.includes('REPLY')) eventType = 'REPLIED';
      else if (rawType.includes('FAIL') || rawType.includes('DROP')) eventType = 'FAILED';
      ...
      await emailTrackingService.processProviderWebhook(
        providerMessageId,
        eventType,
        event,
        { ip: event.ip, useragent: event.useragent, url: event.url }
      );
    }
