...
class WhatsAppService {
  ...
  private setupListeners() {
    this.client.on('qr', async (qr) => {
      try {
        ...
        console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Failed to generate QR code:', getErrorMessage(err));
      }
    });
    ...
    this.client.on('ready', async () => {
      ...
      try {
        ...
        console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] Account: ${this.accountInfo.name} (+${this.accountInfo.number})`);
      } catch (e) {
    ...
    this.client.on('auth_failure', (msg) => {
      console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Authentication failure:', msg);
      this.isConnected = false;
      this.qrCodeDataUrl = null;
    });

    this.client.on('disconnected', (reason) => {
      console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Client disconnected:', reason);
      this.isConnected = false;
      this.qrCodeDataUrl = null;
      this.accountInfo = null;
    });
  ...
  public initialize() {
    console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Starting client initialization...');
    this.client.initialize().catch((err) => {
      console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Initialization error (non-fatal):', getErrorMessage(err));
    });
  }
  ...
  public async logout() {
    try {
      ...
      console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Logout failed:', getErrorMessage(error));
      throw error;
    }
  }
  ...
  public async sendMessage(phoneNumber: string, text: string, mediaPath?: string) {
    ...
    try {
      ...
      if (
        ...
        console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] LID issue for ${chatId}, trying getChat + retry...`);
        ...
        try {
          ...
          console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] Retry also failed for ${phoneNumber}:`, getErrorMessage(retryError));
          throw new Error(getErrorMessage(retryError));
        }
      }

      console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] Failed to send to ${phoneNumber}:`, getErrorMessage(error));
  ...
  private async downloadMediaWithRetry(message: any, retries = 3, delayMs = 1500) {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] downloadMedia attempt ${attempt} failed:`, getErrorMessage(err));
    ...
    try {
      ...
      if (message.hasMedia) {
        if (message.isViewOnce) {
          console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] View-once media — cannot be downloaded, skipping');
        } else {
          try {
            ...
            if (media) {
              ...
              console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Media download unavailable, continuing as text-only');
            }
          } catch (mediaErr) {
            console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Failed to download media:', getErrorMessage(mediaErr));
      ...
      if (fromId.includes('@lid') && (digitsOnly.length > 13 || digitsOnly.length < 10)) {
        try {
          ...
          console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] getContact resolved: ${resolved}, pushname: ${contact?.pushname}`);
        } catch (e) {
          console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] getContact failed:', getErrorMessage(e));
        }
      }

      if (!digitsOnly || digitsOnly.length < 10) {
        console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] Could not get valid phone from message. from=${fromId}`);
        return;
      }

      // Last 10 digits for matching
      const last10 = digitsOnly.slice(-10);
      console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] Incoming reply: "${text}" | phone digits: ${digitsOnly} | last10: ${last10}`);
      ...
      if (existing) {
        console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] Already saved message ${providerMsgId}, skipping.`);
        return;
      }
      ...
      if (!matchedLead) {
        console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] No lead matched last10=${last10}. Leads checked: ${allLeads.length}`);
        return;
      }

      console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] ✅ Matched lead: "${matchedLead.name}" (id=${matchedLead.id})`);
      ...
      console.log('[' + new Date().toISOString() + '] -', `[WhatsApp] ✅ Reply saved for lead "${matchedLead.name}" (id=${matchedLead.id})`);
    } catch (error) {
      console.log('[' + new Date().toISOString() + '] -', '[WhatsApp] Error handling incoming message:', getErrorMessage(error));
    }
  }
}
