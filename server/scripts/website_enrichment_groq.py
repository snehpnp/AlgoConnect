"""
Lead Enrichment Script (Direct Rule-based Scraping) - Extended
-------------------------------------------------------------------------
Fetches leads with a website where scrapedEmail OR scrapedPhone is
missing (NULL or empty string), scrapes the site (main page + relevant
sub-pages), and extracts as many Lead fields as possible using
rule-based heuristics (no LLM).

Safety guarantees:
- COALESCE on every field -> existing non-null DB values are NEVER overwritten.
- sanitize_value/sanitize_result -> empty strings / "None" / "null" / "-"
  are converted to real None before hitting the DB, so we never write
  blank data over NULL columns, and never permanently "poison" a field
  with an empty string that blocks future retries.
- Stale [CLAIMED] leads (crashed workers) get reclaimed after a timeout.
- Leads that fail MAX_ATTEMPTS times are skipped permanently.
- get_total_pending_leads() and claim_one_lead() use the EXACT same
  WHERE criteria, so the "pending count" always matches what's claimable.
"""
