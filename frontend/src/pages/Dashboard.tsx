...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-5 sm:space-y-8 animate-fade-in pb-24 sm:pb-10 px-3 sm:px-6 pt-2 max-w-full overflow-x-hidden">
      {/* Header: stacks cleanly on phone, badges wrap instead of overflowing */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-1 sm:px-0">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold tracking-tight text-[#0F172A]">Dashboard Overview</h1>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1.5 text-[11px] sm:text-xs font-semibold text-primary whitespace-nowrap">
      ...
      {/* 8 Stats Cards - 2 cols on phone, 3 on tablet, 4 on desktop */}
      <div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-4">
        {statCards.map((stat, i) => (
          <div
            key={i}
            onClick={stat.action}
            className="relative overflow-hidden rounded-2xl border border-white/40 bg-white/60 p-3 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] active:scale-[0.98] sm:hover:-translate-y-1 transition-all cursor-pointer group"
          >
            ...
            <div className="flex items-center justify-between relative z-10">
              <div className={`rounded-xl sm:rounded-2xl p-2 sm:p-3 ${stat.bg} shadow-sm group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}>{stat.icon}</div>
              <div className="hidden sm:flex h-8 w-8 items-center justify-center rounded-full bg-slate-50 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
            ...
            <div className="mt-3 sm:mt-5 relative z-10">
              <h3 className="text-[11px] sm:text-sm font-semibold text-slate-500 leading-tight">{stat.name}</h3>
              <div className="mt-1 flex items-baseline gap-2">
                <p className="text-xl sm:text-3xl font-extrabold tracking-tight text-slate-800">{stat.value}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* Analytics + Pipeline + Lead Types: stacks on phone, 2 cols on tablet, full row on desktop */}
      <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-white/40 bg-white/60 p-4 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl md:col-span-2 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm sm:text-base font-bold text-[#0F172A]">Lead Generation Analytics</h2>
              <p className="text-xs text-[#64748B]">Monthly performance overview</p>
            </div>
          </div>
          <div className="mt-6 sm:mt-8 flex h-56 sm:h-64 items-end gap-2 sm:gap-4 px-1 sm:px-2 overflow-x-auto pb-4 custom-scrollbar" style={{ WebkitOverflowScrolling: 'touch' }}>
            {analytics.map((bar, idx) => (
              <div key={idx} className="group flex flex-1 flex-col items-center gap-2 sm:gap-3 min-w-[30px] sm:min-w-[40px]">
                <div className="relative w-full rounded-xl bg-slate-50 overflow-hidden border border-slate-100 transition-all hover:bg-slate-100 hover:border-slate-200 shadow-inner" style={{ height: '160px' }}>
                  <div
                ...
                <span className="text-[9px] sm:text-xs font-bold text-slate-500 group-hover:text-slate-800 transition-colors">{bar.label}</span>
              </div>
            ))}
          </div>
        </div>

        <div className="rounded-2xl border border-white/40 bg-white/60 p-4 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
          <h2 className="text-sm sm:text-base font-bold text-slate-800">Sales Pipeline</h2>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between text-xs gap-2">
                    <span className="font-semibold text-slate-700 group-hover:text-blue-600 transition-colors truncate">{item.status || 'Unassigned'}</span>
        ...
        <div className="rounded-2xl border border-white/40 bg-white/60 p-4 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
          <h2 className="text-sm sm:text-base font-bold text-[#0F172A]">Lead Types</h2>
          ...
          <div className="mt-6 space-y-4 flex flex-col justify-center h-[calc(100%-4rem)]">
            {data?.leadTypes?.map((item, idx) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between text-xs gap-2">
                    <span className="font-semibold text-slate-700 group-hover:text-blue-600 transition-colors truncate">{item.type}</span>
                    <span className="font-bold text-[#0F172A] shrink-0">{item.count}</span>
                  </div>
      ...
      {leaderboard.length > 0 && (
        <div className="rounded-2xl border border-white/40 bg-white/60 p-4 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl mt-6">
          <div className="flex items-center gap-2 mb-5 sm:mb-6">
            <Trophy className="h-5 w-5 sm:h-6 sm:w-6 text-yellow-500 shrink-0" />
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-800">Sales Leaderboard</h2>
              <p className="text-[11px] sm:text-xs text-slate-500">Top performers this month based on converted leads & activity.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
            {leaderboard.map((user, index) => {
              ...
              return (
                <div
                  key={user.id}
                  className={`relative overflow-hidden flex flex-col rounded-xl border p-4 transition-all active:scale-[0.98] sm:hover:-translate-y-1 sm:hover:shadow-lg ${isTop ? 'bg-gradient-to-br from-yellow-50 to-white border-yellow-200' :
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="relative shrink-0">
                        {user.avatar ? (
                          ...
                          <div className={`h-10 w-10 flex items-center justify-center rounded-full text-sm font-bold shadow-sm border-2 border-white ${isTop ? 'bg-yellow-500 text-white' :
                        ...
                        <div className={`absolute -bottom-1 -right-1 h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-bold border-2 border-white ${isTop ? 'bg-yellow-500 text-white' :
                            isSecond ? 'bg-slate-400 text-white' :
                              isThird ? 'bg-orange-400 text-white' :
      ...
      {/* Side-by-Side: Trace and Follow-ups (stacked on phone) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mt-6">
        {/* Communication Trace Report */}
        <div className="rounded-2xl border border-white/40 bg-white/60 p-4 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl flex flex-col hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
          <div className="flex items-center gap-2 mb-4">
            <Activity className="h-5 w-5 text-indigo-600 shrink-0" />
            <h2 className="text-base sm:text-lg font-bold text-[#0F172A]">Recent Communications Trace</h2>
          </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto pr-1" style={{ maxHeight: '400px' }}>
              {recentCommunications.map((event) => {
                ...
                return (
                  <div
                    key={event.id}
                    className="group relative flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-xl border border-white/40 bg-white/60 p-3 sm:p-4 shadow-[0_4px_20px_rgb(0,0,0,0.03)] backdrop-blur-sm hover:shadow-[0_4px_20px_rgb(0,0,0,0.06)] active:scale-[0.99] sm:hover:-translate-y-0.5 transition-all cursor-pointer gap-3 sm:gap-4"
                    onClick={() => navigate(`/leads`)}
                  >
                    <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                      <div className={`p-2.5 sm:p-3 rounded-xl border shadow-sm shrink-0 ${colorClass} group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-300`}>
                        {icon}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <h3 className="font-bold text-slate-800 text-sm truncate max-w-[45vw] sm:max-w-[150px]" title={event.lead.name}>{event.lead.name}</h3>
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${statusColor}`}>
                            {event.eventType}
                          </span>
                        </div>
                        <p className="text-xs font-medium text-slate-500 truncate max-w-[60vw] sm:max-w-[200px]" title={event.campaign?.name || 'Direct Message'}>
                          {event.campaign?.name || 'Direct Message'}
                        </p>
                      </div>
                    </div>

                    <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-2 sm:gap-1 pl-11 sm:pl-0">
                      ...
                      {/* Always visible on touch/mobile; fades in on hover for desktop */}
                      <div className="flex items-center gap-1.5 text-[11px] font-bold text-blue-600 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300 sm:translate-x-2 sm:group-hover:translate-x-0">
                        <span>{nextStep}</span>
        ...
        <div className="rounded-2xl border border-white/40 bg-white/60 p-4 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl flex flex-col h-full hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
          {todaysFollowUps.length > 0 ? (
            <>
              <div className="flex items-center gap-2 mb-5 flex-wrap">
                <CalendarClock className="h-5 w-5 sm:h-6 sm:w-6 text-amber-500 shrink-0" />
                <div className="flex-1 min-w-0">
                  <h2 className="text-base sm:text-lg font-bold text-slate-800">Today's Follow-Ups</h2>
                  <p className="text-[11px] sm:text-xs text-slate-500">{todaysFollowUps.length} lead{todaysFollowUps.length > 1 ? 's' : ''} scheduled for contact today.</p>
                </div>
                {overdueFollowUpsCount > 0 && (
                  <span className="text-[11px] sm:text-xs font-bold bg-red-100 text-red-700 px-2.5 sm:px-3 py-1 rounded-full shrink-0 whitespace-nowrap">
              ...
              <div className="space-y-2 flex-1 overflow-y-auto pr-1" style={{ maxHeight: '400px' }}>
                {todaysFollowUps.map((lead: any) => (
                  <div key={lead.id} onClick={() => navigate(`/leads/${lead.id}`)} className="group flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:border-amber-200 hover:bg-amber-50/30 active:scale-[0.99] cursor-pointer transition-all">
